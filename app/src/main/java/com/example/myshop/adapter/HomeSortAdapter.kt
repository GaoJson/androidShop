...
class HomeSortAdapter : RecyclerView.Adapter<HomeSortAdapter.ViewPageHolder>() {
    ...
    class ViewPageHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        public val adapter = HomeSortAdapter.SortGridItemAdapter()
        private val gridView: RecyclerView = itemView.findViewById(R.id.grid_view)

        init {
            gridView.adapter = adapter
            val layoutManager:GridLayoutManager = object :GridLayoutManager(itemView.context,4, LinearLayoutManager.VERTICAL, false){
